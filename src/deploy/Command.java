...
public class Command extends Thread
{
	...
	public void setCommandMultiple(String command) 
	{
		this.multiple = true;
		this.cmd = command;
	}
	...
	public void run() 
	{
		...
		{
			pb = new ProcessBuilder("bash", "-c", this.cmd);
		}
		else {
			pb = new ProcessBuilder(cmdByWords);
		}
		IO io = new IO(true);
		try {
			String line = "";
			...
			else 
			{
				...
				BufferedReader input = new BufferedReader(new InputStreamReader(p.getInputStream()));
				//getOutputStream pour eviter les bloquages
				boolean b = p.waitFor(30, TimeUnit.SECONDS);
				if (!b)
				{
					response += "TIMEOUT"; 
					p.destroy();
